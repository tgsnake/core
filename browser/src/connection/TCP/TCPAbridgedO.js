...
import { includesBuffer, sliceBuffer, bigintToBuffer } from '../../helpers.js';
import { crypto, Buffer } from '../../platform.browser.js';
import { ctr256Cipher } from '../../crypto/Aes.js';
class TCPAbridgedO extends TCP {
  /** @hidden */
  _reserved;
  /** @hidden */
  _encryptor;
    ...
    if (proxy && 'secret' in proxy && 'port' in proxy && 'server' in proxy && dcId) {
      ...
      while (true) {
        ...
        if (
          ...
          nonce[56] = nonce[57] = nonce[58] = nonce[59] = 239;
          break;
        }
      }
  ...
  async recv(length = 0) {
    ...
    if (_length.equals(Buffer.from('7f', 'hex'))) {
      ...
      return Buffer.from(
        this._decryptor(
          await super.recv(Buffer.concat([_length, Buffer.alloc(1)]).readInt32LE(0) * 4),
        ),
      );
    }
    return Buffer.from(this._decryptor(await super.recv(_length[0] * 4)));
  }
}
