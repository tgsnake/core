...
import { SecretChat as TempChat } from '../../storage/index.js';
import { Mutex, inspect, crypto, Buffer } from '../../platform.browser.js';
...
class SecretChat {
  ...
  constructor(storage, client) {
    ...
    this._tempAuthKey = /* @__PURE__ */ new Map();
    this._waiting = [];
  }
  /**
  ...
  async start(userId) {
    ...
    try {
      await TempChat.save(this._storage, {
        ...
        isAdmin: false,
        // set false in here, because we not finished to create the secret chat.
      });
    } finally {
      release();
    }
  ...
  async commitRekeying(chatId, action) {
    ...
    const authKey = await toBuffer(
      bigIntPow(gB, await toBigint(this._tempAuthKey.get(action.exchangeId)), p),
      256,
      false,
    );
    const fingerprint = sha1(authKey).slice(-8).readBigInt64LE();
  ...
  async finalRekeying(chatId, action) {
    ...
    const fingerprint = sha1(this._tempAuthKey.get(action.exchangeId)).slice(-8).readBigInt64LE();
    if (fingerprint !== action.keyFingerprint) {
  ...
  async decrypt(message) {
    ...
    if (!this._waiting.includes(message.chatId)) {
      ...
      if (
        (peer.timeRekey <= 0 || Date.now() / 1e3 - peer.changed < 7 * 24 * 60 * 60) &&
        peer.rekeyStep === 0
      ) {
